...
object NaiveBayes{
  ...
  def validate(): Unit ={
    ...
    for (k <- -8 until -15 by -1) {
      ...
      logger.log("k=" + k + " F1-Average=" + validationF1.sum / validationF1.length)
    }
  }
